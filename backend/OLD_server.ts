/**
 * Backend API Routes for Product Service (Node.js/Express)
 * This API connects to MySQL database and provides CRUD operations for products
 */
...
/**
 * GET /api/Get - Fetch all products
 */
...
app.get('/api/products/:id', async (req: Request, res: Response) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [rows] = await connection.query(
      'SELECT id, name, description, price, category, imageUrl, rating FROM estore.product_list_items WHERE id = ?',
      [id]
    );
    connection.release();
...
app.get('/api/products/category/:category', async (req: Request, res: Response) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [rows] = await connection.query(
      'SELECT id, name, description, price, category, imageUrl, rating FROM estore.product_list_items WHERE category = ?',
      [category]
    );
    connection.release();
    res.json(rows);
...
app.post('/api/products', async (req: Request, res: Response) => {
  try {
    ...
    const [result] = await connection.query(
      'INSERT INTO estore.product_list_items (name, description, price, category, imageUrl, rating) VALUES (?, ?, ?, ?, ?, ?)',
...
app.delete('/api/products/:id', async (req: Request, res: Response) => {
  try {
    ...
    await connection.query('DELETE FROM estore.product_list_items WHERE id = ?', [id]);
    connection.release();
